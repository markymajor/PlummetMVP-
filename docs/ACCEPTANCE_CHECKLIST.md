...
- The two instruction screens appear only when the game is opened for the first time on that device/profile.
- Returning home from the RESCUED screen does not show the instruction screens again.
...
- Touching a wall or obstacle triggers the end state.
- The RESCUED screen shows `rescued-title.png`, `rescue-firefighter.png`, the selected falling player, reset, home, and share buttons.
